...
"#viewportjs-div-test-element {width:10px !important}}</style>"].join("");b.appendChild(a);g.insertBefore(b,c);a=a.offsetWidth===10?e:d;g.removeChild(b)}return a}();p.prototype={is:function(a){return this.current().name===a},current:function(){for(var a=m,b=0,c=this.viewports.length;b<c;b++){var d=this.viewports[b],e=d.name;if(this.vps[e])if(this.vps[e].test())a=d}return a},matches:function(a){return this.vps[a]&&this.vps[a].test()},subscribe:function(a,b){this.state.tokenUid+=1;this.state.channels[a]||
(this.state.channels[a]=[]);this.state.channels[a].push({token:this.state.tokenUid,method:b});this.state.present.name===a&&b(true,this.state.present);return this.state.tokenUid},unsubscribe:function(a){var b,c;for(c in this.state.channels)if(b=this.state.channels[c])for(var d=0,e=b.length;d<e;d++)b[d].token===a&&b.splice(d,1)},publish:function(a,b){var c=this.state.channels[a],d=c?c.length:0;if(c)for(;d--;)c[d].method(b,this.vps[a])},update:function(){if(this.state.initialUpdate||l){if(this.state.initialUpdate)this.state.initialUpdate=
false;this.state.previous=this.state.present;this.state.present=this.current()}if(this.state.present!==this.state.previous){this.publish(this.state.previous.name,false);this.publish(this.state.present.name,true)}}};if("addEventListener"in window){window.addEventListener("resize",f);window.addEventListener("orientationchange",f)}else window.attachEvent("onresize",f);j();return function(a){a=new p(a);h.push(a);a.update();return{vps:a.vps,viewports:a.viewports,is:i(a,a.is),current:i(a,a.current),matches:i(a,
a.matches),subscribe:i(a,a.subscribe),unsubscribe:i(a,a.unsubscribe)}}});